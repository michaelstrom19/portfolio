...
//fixed seed
float SEED = 120.5; 
const int NUM_SPHERES = 16; //limit number of spheres so it can load lol
sphere random_spheres[NUM_SPHERES]; 

void create_random_scene() {
    init_rand(vec2(SEED, SEED), 0.0);

    material ground_material = material(vec3(0.5, 0.5, 0.5), MATERIAL_LAMBERTIAN, 0.0, 0.0);
    material material_metal  = material(vec3(0.8, 0.6, 0.2), MATERIAL_METAL, 0.02, 0.0); 

    random_spheres[0] = sphere(vec3(0.0, -1000.0, 0.0), 1000.0, ground_material);
    random_spheres[1] = sphere(vec3(0.0, 3.0, 0.0), 2.0, material_metal);
    material glass_material = material(vec3(1.0, 1.0, 1.0), MATERIAL_DIELECTRIC, 0.0, 1.5);

    int index = 2;
    for (int a = -5; a < 5; a++) {
        for (int b = -5; b < 5; b++) {
            float choose_type = rand1(SEED);
            vec3 center = vec3(0.0, 2.0* rand1(SEED), 0.0);

            if (choose_type < 0.3) {
                random_spheres[index++] = sphere(center, 0.5* rand1(SEED), glass_material);
            } else {
                random_spheres[index++] = sphere(center, -0.2* rand1(SEED), glass_material);
            }
        }
    }
}
...
vec3 ray_color(const ray r, int depth) {
    ...
    sphere spheres[NUM_SPHERES + 1]; // +1 for the ground sphere
    ...
    for (int i = 0; i < depth; i++) {
        ...
        if (material_scatter(rec.mat, current_ray, rec, attenuation, scattered)) {
            ...
            vec3 background = (1.0 - a) * vec3(1.0, 0.4, 1.0) + a * vec3(1.0, 0.2, .1);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
 {
    ...
    // Camera setup
    vec3 lookfrom = vec3(5, 1, 5);
    vec3 lookat = vec3(0, 1, 0);
    vec3 vup = vec3(0, 1, 0);
    float vfov = 20.0;
    float aperture = 0.02;
    float focus_dist = length(lookfrom - vec3(0.0, 1.0, 0.0));
